...
def conll_to_train(conll, vocab):
  return [(x[1], vocab.stoi(x[1]), x[6]) for x in conll]
...
class Encoder(Chain):
  ...
  def __call__(self, x_list):
    ...
    f_list = []
    for x in x_list:
      f_list.append(self.x_f(x))
    b_list = []
    for x in reversed(x_list):
      b_list.insert(0, self.x_b(x))
    y_list = []
...
class StackedEncoder(ChainList):
  ...
  def __call__(self, x_list):
    y_list = self[0](x_list)
    y_list = self[1](y_list)
    #y_list = self[2](y_list)
    return y_list
...
class Parser(Chain):
  def __init__(self, vocab_size, embed_size, hidden_size):
    super(Parser, self).__init__(
      ...
      p_encode = StackedEncoder(embed_size, hidden_size),
      c_encode = StackedEncoder(embed_size, hidden_size),
      r_encode = StackedEncoder(embed_size, hidden_size),
  ...
  def prepare(self):
    self.p_embed.prepare()
    self.c_embed.prepare()
    self.r_embed.prepare()
...
def train(args):
  ...
  for epoch in range(args.epoch):
    ...
    for i, data in enumerate(train_dataset):
      ...
      root = -1
      for j, (p_scores, (text, wid, parent)) in enumerate(zip(parent_scores, data)):
        if parent == -1:
          trace('  %3d: root' % j)
          root = j
        else:
