...
class StackSequence(Chain):
  def __init__(self, n_stack, n_label):
    super(StackSequence, self).__init__(
      w_xy = links.Linear(n_stack, 4 * n_stack),
      w_ly = links.EmbedID(n_label, 4 * n_stack),
      w_yy = links.Linear(n_stack, 4 * n_stack),
    )

  def __call__(self, c, x, l, y):
    return functions.lstm(
      c,
      self.w_xy(x) + self.w_ly(l) + self.w_yy(y),
    )
...
class Parser(Chain):
  def __init__(
    ...
    super(Parser, self).__init__(
      ...
      net_stack = StackSequence(n_stack, n_semiterminal + n_phrase),
  ...
  def forward(self, word_list, gold_op_list, unary_limit):
    ...
    for i in itertools.count():
      ...
      label_id = label_argmax + (self.n_phrase if o_argmax == OP_SHIFT else 0)
      rc0, r0 = self.net_stack(rc_prev, s0, XP.iarray([label_id]), r_prev)
