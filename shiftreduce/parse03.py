...
class WordEmbedding2(Chain):
  ...
  def __call__(self, word):
    x_list = [self.net_embed(my_array([[min(ord(c), 127)]], np.int32)) for c in word]
    c = my_zeros((1, self.n_char_encode), np.float32)
    f = my_zeros((1, self.n_char_encode), np.float32)
    for x in x_list:
      c, f = self.net_forward(c, x, f)
    c = my_zeros((1, self.n_char_encode), np.float32)
    b = my_zeros((1, self.n_char_encode), np.float32)
...
class Parser(Chain):
  def __init__(self,
    ...
    super(Parser, self).__init__(
        ...
        net_embed2 = WordEmbedding2(16, 32),
        net_encoder = LinearEncoder(n_embed, n_queue_state),
        net_operation = OperationEstimator(n_embed, 32, n_queue_state, n_stack_state),
        net_shift = ShiftTransition(n_embed, 32, n_queue_state, n_stack_state),
  ...
  def forward(self, word_list, op_list, unary_limit):
    ...
    EMBED2_ZEROS = my_zeros((1, self.n_embed2), np.float32)
