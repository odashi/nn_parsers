...
def make_tree(text):
  return Tree.fromstring(text)[0] # ignore ROOT
  ...
  def recursive(tree):
    if isinstance(tree, Tree):
      if len(tree) == 1:
        ...
        # binary and more
        ret = recursive(tree[0])
        for ch in tree[1:-1]:
          ret += recursive(ch) + [(OP_PARTIAL, tree.label())]
        return ret + recursive(tree[-1]) + [(OP_REDUCE, tree.label())]
...
def convert_op_list(op_list, phrase_vocab, semi_vocab):
  ...
  for op, label in op_list:
    ...
    elif op in [OP_UNARY, OP_PARTIAL, OP_REDUCE]:
...
def unbinarize(tree):
  if isinstance(tree, Tree):
    children = [unbinarize(ch) for ch in tree]
    if isinstance(children[0], Tree) and children[0].label() == -1:
      children = [gch for gch in children[0]] + children[1:]
...
class Parser(Chain):
  def __init__(self,
    ...
    super(Parser, self).__init__(
        ...
        net_partial = BinaryTransition(n_queue_state, n_stack_state),
        net_reduce = BinaryTransition(n_queue_state, n_stack_state),
        net_partial_label = LabelEstimator(n_stack_state, n_phrase_label),
