...
class CharEmbedding(Chain):
  def __init__(self, n_char_embed):
    ...
    self.__char = [XP.iarray([i]) for i in range(0x80)]

  def reset(self):
    self.__v_xa = [self.w_xa(x) for x in self.__char]
    self.__v_xb = [self.w_xb(x) for x in self.__char]
...
class QueueSequence(Chain):
  def __init__(self, n_embed, n_char_embed, n_output):
    super(QueueSequence, self).__init__(
      w_xy = links.Linear(n_embed, 4 * n_output, nobias=True),
      w_ay = links.Linear(n_char_embed, 4 * n_output, nobias=True),
      w_by = links.Linear(n_char_embed, 4 * n_output, nobias=True),
...
class ShiftReduceSequence(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(ShiftReduceSequence, self).__init__(
      w_az = links.Linear(n_queue, 4 * n_srstate, nobias=True),
      w_bz = links.Linear(n_queue, 4 * n_srstate, nobias=True),
      w_s1z = links.Linear(n_stack, 4 * n_srstate, nobias=True),
      w_s2z = links.Linear(n_stack, 4 * n_srstate, nobias=True),
...
class ShiftTransition(Chain):
  def __init__(self, n_embed, n_char_embed, n_queue, n_stack, n_srstate):
    super(ShiftTransition, self).__init__(
      w_xs = links.Linear(n_embed, n_stack, nobias=True),
      w_xas = links.Linear(n_char_embed, n_stack, nobias=True),
      w_xbs = links.Linear(n_char_embed, n_stack, nobias=True),
      w_qas = links.Linear(n_queue, n_stack, nobias=True),
      w_qbs = links.Linear(n_queue, n_stack, nobias=True),
      w_s1s = links.Linear(n_stack, n_stack, nobias=True),
...
class UnaryTransition(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(UnaryTransition, self).__init__(
      w_as = links.Linear(n_queue, 4 * n_stack, nobias=True),
      w_bs = links.Linear(n_queue, 4 * n_stack, nobias=True),
      w_s1s = links.Linear(n_stack, 4 * n_stack, nobias=True),
      w_s2s = links.Linear(n_stack, 4 * n_stack, nobias=True),
...
class BinaryTransition(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(BinaryTransition, self).__init__(
      w_as1 = links.Linear(n_queue, 4 * n_stack, nobias=True),
      w_bs1 = links.Linear(n_queue, 4 * n_stack, nobias=True),
      w_s1s1 = links.Linear(n_stack, 4 * n_stack, nobias=True),
      w_s2s1 = links.Linear(n_stack, 4 * n_stack, nobias=True),
      w_zs1 = links.Linear(n_srstate, 4 * n_stack),
      w_s1s2 = links.Linear(n_stack, 4 * n_stack, nobias=True),
      w_s2s2 = links.Linear(n_stack, 4 * n_stack, nobias=True),
      w_s3s2 = links.Linear(n_stack, 4 * n_stack, nobias=True),
