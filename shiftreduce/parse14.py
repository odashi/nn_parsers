...
class LabelEstimator(Chain):
  def __init__(self, n_queue, n_stack, n_output):
    super(LabelEstimator, self).__init__(
        w_qay = links.Linear(n_queue, n_output),
        w_qby = links.Linear(n_queue, n_output),
        w_s1y = links.Linear(n_stack, n_output),
        w_s2y = links.Linear(n_stack, n_output),
    )

  def __call__(self, qa, qb, s1, s2):
    return self.w_qay(qa) + self.w_qby(qb) + self.w_s1y(s1) + self.w_s2y(s2)

class LabelEstimator2(Chain):
  def __init__(self, n_input, n_output):
    super(LabelEstimator2, self).__init__(
...
class Parser(Chain):
  def __init__(
    ...
    super(Parser, self).__init__(
      ...
      net_operation = LabelEstimator2(n_srstate, NUM_OP),
      net_phrase = LabelEstimator(n_queue, n_stack, n_phrase),
      net_semiterminal = LabelEstimator(n_queue, n_stack, n_semiterminal),
    )
