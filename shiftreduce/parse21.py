...
class ShiftReduceSequence(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(ShiftReduceSequence, self).__init__(
      ...
      w_oz = links.EmbedID(NUM_OP, 4 * n_srstate),
    )

  def __call__(self, c, a, b, s1, r1, s2, r2, z, o):
    return functions.lstm(
      c,
      self.w_az(a) + self.w_bz(b) + self.w_s1z(s1) + self.w_r1z(r1) + \
      self.w_s2z(s2) + self.w_r2z(r2) + self.w_zz(z) + self.w_oz(o),
    )
...
class Parser(Chain):
  ...
  def forward(self, word_list, gold_op_list, unary_limit):
    ...
    for i in itertools.count():
      ...
      zc, z = self.net_sr(zc, a, b, s1, r1, s2, r2, z, XP.iarray([o_argmax]))
