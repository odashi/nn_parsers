...
class ShiftTransition(Chain):
  ...
  def __call__(self, q, s1, z):
    return functions.tanh(self.w_qs(q) + self.w_s1s(s1) + self.w_zs(z))
...
class Parser(Chain):
  def __init__(self, n_vocab, n_queue, n_stack, n_srstate, n_phrase, n_semiterminal):
    super(Parser, self).__init__(
      net_encoder = LinearEncoder(n_vocab, n_queue),
      net_sr = LinearEncoder2(n_queue, n_stack, n_srstate),
      net_shift = ShiftTransition(n_queue, n_stack, n_srstate),
  ...
  def load_spec(filename):
    with open(filename) as fp:
      ...
      return Parser(n_vocab, n_queue, n_stack, n_srstate, n_phrase, n_semiterminal)
