...
class ShiftReduceSequence(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(ShiftReduceSequence, self).__init__(
      w_qz = links.Linear(n_queue, 4 * n_srstate),
  ...
  def __call__(self, c, q, s1, r1, s2, r2, z):
    return functions.lstm(
      c,
      self.w_qz(q) + self.w_s1z(s1) + self.w_r1z(r1) + \
...
class ShiftTransition(Chain):
  def __init__(self, n_embed, n_char_embed, n_queue, n_stack):
    super(ShiftTransition, self).__init__(
      ...
      w_as = links.Linear(n_char_embed, n_stack),
      w_bs = links.Linear(n_char_embed, n_stack),
      w_qs = links.Linear(n_queue, n_stack),
      w_s1s = links.Linear(n_stack, n_stack),
    )

  def __call__(self, x, a, b, q, s1):
    return functions.tanh(
      self.w_xs(x) + self.w_as(a) + self.w_bs(b) + self.w_qs(q) + self.w_s1s(s1),
    )
...
class Parser(Chain):
  ...
  def forward(self, word_list, gold_op_list, unary_limit):
    ...
    for x, (j, k) in zip(reversed(x_list), reversed(jk_list)):
      ac, a = self.net_backward(ac, x, j, k, a)
      a_list.insert(0, a)

    q_list = [
      (text, x, j, k, a) \
      for (text, _), x, (j, k), a \
      in zip(word_list, x_list, jk_list, a_list)]
    ...
    for i in itertools.count():
      text, x, j, k, a = q_list[0] if q_list else QUEUE_DEFAULT
      ...
      zc, z = self.net_sr(zc, a, s1, r1, s2, r2, z)  
      ...
      if o_argmax == OP_SHIFT:
        t0 = Tree(None, [text])
        sc0, s0 = (STACK_ZEROS, self.net_shift(x, j, k, a, s1))
        ...
        t0 = Tree(None, [t1])
        sc0, s0 = self.net_unary(sc1, a, s1, s2)
        ...
        t0 = Tree(None, [t2, t1])
        sc0, s0 = self.net_binary(sc1, sc2, a, s1, s2, s3)
...
def train(args):
  ...
  opt.add_hook(optimizer.WeightDecay(0.00001))
