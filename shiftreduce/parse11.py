...
class ShiftReduceSequence(Chain):
  ...
  def __call__(self, c, a, b, s1, z):
    return functions.lstm(
      c,
      self.w_az(a) + self.w_bz(b) + self.w_s1z(s1) + self.w_zz(z),
    )

class ShiftTransition(Chain):
  def __init__(self, n_embed, n_queue, n_stack, n_srstate):
    super(ShiftTransition, self).__init__(
      w_xs = links.Linear(n_embed, n_stack),
      w_as = links.Linear(n_queue, n_stack),
      w_bs = links.Linear(n_queue, n_stack),
      w_s1s = links.Linear(n_stack, n_stack),
      w_zs = links.Linear(n_srstate, n_stack),
    )

  def __call__(self, x, a, b, s1, z):
    return functions.tanh(
      self.w_xs(x) + self.w_as(a) + self.w_bs(b) + self.w_s1s(s1) + self.w_zs(z),
    )
...
class Parser(Chain):
  def __init__(self, n_vocab, n_embed, n_queue, n_stack, n_srstate, n_phrase, n_semiterminal):
    super(Parser, self).__init__(
      net_embed = Embedding(n_vocab, n_embed),
      net_forward = LinearEncoder(n_embed, n_queue),
      net_backward = LinearEncoder(n_embed, n_queue),
      net_sr = ShiftReduceSequence(n_queue, n_stack, n_srstate),
      net_shift = ShiftTransition(n_embed, n_queue, n_stack, n_srstate),
  ...
  def load_spec(filename):
    with open(filename) as fp:
      ...
      return Parser(n_vocab, n_embed, n_queue, n_stack, n_srstate, n_phrase, n_semiterminal)
