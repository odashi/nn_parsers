...
class LinearEncoder2(Chain):
  def __init__(self, n_input1, n_input2, n_output):
    super(LinearEncoder2, self).__init__(
      w_x1y = links.Linear(n_input1, 4 * n_output),
      w_x2y = links.Linear(n_input2, 4 * n_output),
      w_yy = links.Linear(n_output, 4 * n_output),
    )
...
class ShiftTransition(Chain):
  ...
  def __call__(self, x, q, s1, z):
    return functions.tanh(self.w_xs(x) + self.w_qs(q) + self.w_s1s(s1) + self.w_zs(z))
...
class Parser(Chain):
  def __init__(self, n_vocab, n_embed, n_queue, n_stack, n_srstate, n_phrase, n_semiterminal):
    super(Parser, self).__init__(
      ...
      net_encoder = LinearEncoder(n_embed, n_queue),
      net_sr = LinearEncoder2(n_queue, n_stack, n_srstate),
