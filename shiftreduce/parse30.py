...
def to_vram_words(word_list):
  return [(text, XP.iarray([wid])) for text, wid in word_list]

def to_vram_ops(op_list):
  return [(op, label, XP.iarray([op]), (XP.iarray([label]) if op != OP_FINISH else None)) for op, label in op_list]
...
class CharEmbedding(Chain):
  def __init__(self, n_char_embed):
    ...
    self.__char_vram = [XP.iarray([i]) for i in range(0x80)]
...
class QueueSequence(Chain):
  def __init__(self, n_embed, n_char_embed, n_output):
    super(QueueSequence, self).__init__(
      w_xu = links.Linear(n_embed, n_output),
      w_au = links.Linear(n_char_embed, n_output),
      w_bu = links.Linear(n_char_embed, n_output),
      w_uy = links.Linear(n_output, 4 * n_output),
...
class ShiftReduceSequence(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(ShiftReduceSequence, self).__init__(
      w_au = links.Linear(n_queue, n_srstate),
      w_bu = links.Linear(n_queue, n_srstate),
      w_s1u = links.Linear(n_stack, n_srstate),
      w_r1u = links.Linear(n_stack, n_srstate),
      w_s2u = links.Linear(n_stack, n_srstate),
      w_r2u = links.Linear(n_stack, n_srstate),
      w_uz = links.Linear(n_srstate, 4 * n_srstate),
...
class ShiftTransition(Chain):
  def __init__(self, n_embed, n_char_embed, n_queue, n_stack):
  ...
  def __call__(self, x, xa, xb, qa, qb, s1):
    return XP.dropout(functions.tanh(
      self.w_xs(x) + self.w_xas(xa) + self.w_xbs(xb) + \
      self.w_qas(qa) + self.w_qbs(qb) + self.w_s1s(s1),
    ))

class UnaryTransition(Chain):
  def __init__(self, n_queue, n_stack):
    super(UnaryTransition, self).__init__(
      w_au = links.Linear(n_queue, n_stack),
      w_bu = links.Linear(n_queue, n_stack),
      w_s2u = links.Linear(n_stack, n_stack),
      w_us = links.Linear(n_stack, 4 * n_stack),
...
class BinaryTransition(Chain):
  def __init__(self, n_queue, n_stack):
    super(BinaryTransition, self).__init__(
      w_au1 = links.Linear(n_queue, n_stack),
      w_bu1 = links.Linear(n_queue, n_stack),
      w_s2u1 = links.Linear(n_stack, n_stack),
      w_u1s1 = links.Linear(n_stack, 4 * n_stack),
      w_s1s1 = links.Linear(n_stack, 4 * n_stack),
      w_s1u2 = links.Linear(n_stack, n_stack),
      w_s3u2 = links.Linear(n_stack, n_stack),
      w_u2s2 = links.Linear(n_stack, 4 * n_stack),
...
class Parser(Chain):
  def __init__(
    ...
    super(Parser, self).__init__(
      ...
      net_stack = StackSequence(n_stack),
      net_sr = ShiftReduceSequence(n_queue, n_stack, n_srstate),
      net_shift = ShiftTransition(n_embed, n_char_embed, n_queue, n_stack),
    ...
    # default values
    self.EMBED_ZEROS = XP.fzeros((1, self.n_embed))
    self.CEMBED_ZEROS = XP.fzeros((1, self.n_char_embed))
    self.QUEUE_ZEROS = XP.fzeros((1, self.n_queue))
    self.STACK_ZEROS = XP.fzeros((1, self.n_stack))
    self.SRSTATE_ZEROS = XP.fzeros((1, self.n_srstate))
    self.QUEUE_DEFAULT = ('', self.EMBED_ZEROS, self.CEMBED_ZEROS, self.CEMBED_ZEROS, self.QUEUE_ZEROS, self.QUEUE_ZEROS)
    self.STACK_DEFAULT = (None, self.STACK_ZEROS, self.STACK_ZEROS, self.STACK_ZEROS, self.STACK_ZEROS)
    self.NEG_INF = -1e20
