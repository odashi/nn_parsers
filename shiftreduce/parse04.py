...
class WordEmbedding2(Chain):
  def __init__(self, n_char_embed, n_char_encode):
    super(WordEmbedding2, self).__init__(
      net_embed = WordEmbedding(128, n_char_embed),
      net_forward = LinearEncoder(n_char_embed, n_char_encode),
      net_backward = LinearEncoder(n_char_embed, n_char_encode),
    )
    self.n_char_encode = n_char_encode

  def __call__(self, word):
    x_list = [self.net_embed(XP.iarray([[min(ord(c), 127)]])) for c in word]
    c = XP.fzeros((1, self.n_char_encode))
    f = XP.fzeros((1, self.n_char_encode))
    for x in x_list:
      c, f = self.net_forward(c, x, f)
    c = XP.fzeros((1, self.n_char_encode))
    b = XP.fzeros((1, self.n_char_encode))
    for x in reversed(x_list):
      c, b = self.net_forward(c, x, b)
    return f, b

class OperationEstimator(Chain):
  def __init__(self, n_embed, n_embed2, n_queue_state, n_stack_state):
    super(OperationEstimator, self).__init__(
        w_xo = links.Linear(n_embed, NUM_OP),
        w_fo = links.Linear(n_embed2, NUM_OP),
        w_bo = links.Linear(n_embed2, NUM_OP),
  ...
  def __call__(self, x, f, b, q, s1, s2, s3):
    return (
        self.w_xo(x) + self.w_qo(q) + self.w_fo(f) + self.w_bo(b) +
        self.w_s1o(s1) + self.w_s2o(s2) + self.w_s3o(s3)
    )
...
class ShiftTransition(Chain):
  def __init__(self, n_embed, n_embed2, n_queue_state, n_stack_state):
    super(ShiftTransition, self).__init__(
        ...
        w_fs = links.Linear(n_embed2, n_stack_state),
        w_bs = links.Linear(n_embed2, n_stack_state),
        w_qs = links.Linear(n_queue_state, n_stack_state),
        w_s1s = links.Linear(n_stack_state, n_stack_state),
    )

  def __call__(self, x, f, b, q, s1):
    return functions.tanh(
        self.w_xs(x) + self.w_fs(f) + self.w_bs(b) +
        self.w_qs(q) + self.w_s1s(s1)
    )
...
class Parser(Chain):
  def __init__(self,
      n_vocab, n_embed, n_vocab2, n_embed2,
      n_queue_state, n_stack_state,
      n_phrase_label, n_semi_label):
    super(Parser, self).__init__(
        net_embed = WordEmbedding(n_vocab, n_embed),
        net_embed2 = WordEmbedding2(n_vocab2, n_embed2),
        net_encoder = LinearEncoder(n_embed, n_queue_state),
        net_operation = OperationEstimator(n_embed, n_embed2, n_queue_state, n_stack_state),
        net_shift = ShiftTransition(n_embed, n_embed2, n_queue_state, n_stack_state),
  ...
  def save_spec(self, filename):
    with open(filename, 'w') as fp:
      ...
      print(self.n_vocab2, file=fp)
      print(self.n_embed2, file=fp)
  ...
  def load_spec(filename):
    with open(filename) as fp:
      n_vocab = int(next(fp))
      n_embed = int(next(fp))
      n_vocab2 = int(next(fp))
      n_embed2 = int(next(fp))
  ...
  def forward(self, word_list, op_list, unary_limit):
    ...
    for i in itertools.count():
      text, x, f, b, q = xfbq_list[0] if xfbq_list else ('', EMBED_ZEROS, EMBED2_ZEROS, EMBED2_ZEROS, QUEUE_ZEROS)
      ...
      if is_training:
        loss += functions.softmax_cross_entropy(op, XP.iarray([op_list[i][0]]))
      ...
      if is_training:
        loss += functions.softmax_cross_entropy(label, XP.iarray([op_list[i][1]]))
...
def train(args):
  ...
  parser = Parser(
      args.vocab, args.embed,
      32, 64,
      args.queue, args.stack,
