...
class ShiftReduceSequence(Chain):
  def __init__(self, n_queue, n_stack, n_srstate):
    super(ShiftReduceSequence, self).__init__(
      ...
      w_r1z = links.Linear(n_stack, 4 * n_srstate),
      w_zz = links.Linear(n_srstate, 4 * n_srstate),
    )

  def __call__(self, c, b, r1, z):
    c, h = functions.lstm(
      c,
      self.w_bz(b) + self.w_r1z(r1) + self.w_zz(z),
    )
    return c, XP.dropout(h)
...
class UnaryTransition(Chain):
  ...
  def __call__(self, c, b, s1, s2):
    c, h = functions.lstm(
      c,
      self.w_bs(b) + self.w_s1s(s1) + self.w_s2s(s2),
    )
    return c, XP.dropout(h)
...
class Parser(Chain):
  def __init__(
    ...
    self.QUEUE_DEFAULT = ('', self.EMBED_ZEROS, self.CEMBED_ZEROS, self.CEMBED_ZEROS, self.QUEUE_ZEROS)
  ...
  def forward(self, word_list, gold_op_list, unary_limit):
    ...
    # word embedding
    x_list = [self.net_embed(wid) for _, wid in word_list]
    ...
    for i in itertools.count():
      ...
      text, x, j, k, b = q_list[0] if q_list else self.QUEUE_DEFAULT
      ...
      if o_argmax == OP_SHIFT:
        t0 = Tree(None, [text])
        sc0, s0 = (self.STACK_ZEROS, self.net_shift(x, j, k, b, s1))
