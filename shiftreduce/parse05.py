...
class OperationEstimator(Chain):
  def __init__(self, n_queue, n_stack):
    super(OperationEstimator, self).__init__(
      w_qo = links.Linear(n_queue, 4 * NUM_OP),
      w_so = links.Linear(n_stack, 4 * NUM_OP),
      w_oo = links.Linear(NUM_OP, 4 * NUM_OP),
    )

  def __call__(self, c, q, s, o):
    return functions.lstm(c, self.w_qo(q) + self.w_so(s) + self.w_oo(o))
...
class ShiftTransition(Chain):
  ...
  def __call__(self, c, q, s1):
    return functions.lstm(c, self.w_qs(q) + self.w_s1s(s1))
...
class UnaryTransition(Chain):
  ...
  def __call__(self, c, s1, s2):
    return functions.lstm(c, self.w_s1s(s1) + self.w_s2s(s2))
...
class BinaryTransition(Chain):
  def __init__(self, n_stack):
    super(BinaryTransition, self).__init__(
      w_s1s = links.Linear(n_stack, 4 * n_stack),
      w_s2s = links.Linear(n_stack, 4 * n_stack),
      w_s3s = links.Linear(n_stack, 4 * n_stack),
    )

  def __call__(self, c, s1, s2, s3):
    return functions.lstm(c, self.w_s1s(s1) + self.w_s2s(s2) + self.w_s3s(s3))
...
class Parser(Chain):
  def __init__(self, n_vocab, n_queue, n_stack, n_phrase, n_semiterminal):
    super(Parser, self).__init__(
      ...
      net_unary = UnaryTransition(n_stack),
      net_binary = BinaryTransition(n_stack),
